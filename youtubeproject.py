...
#creating engine 
engine=create_engine('mysql+pymysql://root:Naveenk02@localhost/youtube',echo=False)

#cur.execute("create database youtube")
myconnection=pymysql.connect(host='127.0.0.1',user='root',passwd='Naveenk02',database='youtube')
...
def api_connect():
    ...
    api_version = 'v3'
    API_KEY ='AIzaSyBSlS49beX6vdhGk9CKWTFNW4fg9sVLQrM'
...
if selected == "Home":
    ...
    st.markdown("## :orange[Technologies used]: Python,MongoDB, Youtube Data API, MySql, Streamlit these are the technologies used in this project")
    st.markdown("## :blue[Overview] : First we have to fetch the data from the youtube by using google youtube api. After fetching the data from the youtube we have to push the data into mongodb. Once we finished these stpes the again we have to fetch the data from the mongodb and make it as a table formet then insert into the MySql")
